/**
 * Mock Support API (add-on surface — beyond build.md's F0–F7 plan).
 *
 * The real support platform will own tickets server-side; this module
 * mirrors that surface with the same network-shaped signatures. Workflow
 * rules live HERE (the mock server), never in components:
 *
 *  - Learner isolation: a learner can only read their own tickets (404 for
 *    anyone else's) — enforced server-side.
...
export async function listMyTickets(
  ...
  if (userId === BOOM) {
    throw new MockApiError(
      "support_down",
      "Support backend unreachable (simulated).",
      503,
    );
  }
 ...
 * notes included (agents see everything). Role-gated server-side.
 */
...
/** Agent-only. Enforced server-side (403 for learners) + audit-logged. */
