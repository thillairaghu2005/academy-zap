...
export const surfaces: SurfaceMeta[] = [
  ...
  },
  {
    ...
    description:
      "Provision a session, drive a real xterm.js terminal over a mock WebSocket, and check objectives off live.",
    planned: [
  ...
  },
  {
    ...
    description:
      "Timed question flow with a live combo meter — the dopamine layer — and stubbed anti-cheat telemetry hooks.",
    planned: [
  ...
  },
  {
    ...
    description:
      "All 10 rank levels plus Prestige tiers, dual XP tracks shown as two distinct bars, streaks and momentum.",
    planned: [
  ...
  },
  {
    ...
    description:
      "Paginated leaderboard rows shaped like Redis ZRANGE reads, with league promotion/relegation zones.",
    planned: [
      "Global + guild leaderboards with mock ZRANGE-shaped pagination",
      "Season/league standing: tier badge (bronze → obsidian), promotion/relegation zones",
      "XP history line chart (recharts) over mock ledger data",
    ],
    notBuiltYet: [
  ...
  },
  {
    ...
    description:
      "Guild member lists, combined XP rollups, and guild-vs-guild comparisons from GuildRollup contracts.",
    planned: [
      "Guild board: member list + combined XP rollup",
      "Guild-vs-guild comparison view",
      "Skill tree visualization (d3 force/tree) over category XP",
    ],
    notBuiltYet: [
  ...
  },
  {
    ...
    description:
      "Cart and checkout rendered around Razorpay/Stripe hosted checkout embeds — never a custom card field.",
    planned: [
      "Cart + checkout page using hosted checkout embeds (sandbox)",
      "Entitlement gating UI for unpurchased courses/labs",
      "Subscription / B2B seat management screens (mock)",
    ],
    notBuiltYet: [
      "Custom card-number input of any kind — PCI scope stays with the providers",
      "Real payment capture (Commerce backend deferred)",
    ],
  },
  {
    slug: "admin",
    ...
    tagline: "Authoring, review & audit (lowest priority)",
    description:
      "Course authoring with draft vs published, the two-person review flow, and append-only audit logs.",
    planned: [
      "Course authoring: draft editor + preview link",
      "Submit-for-review → second-reviewer publish flow",
      "Moderation / audit-log view (mock append-only rendering)",
    ],
    notBuiltYet: [
      "Real content versioning enforcement (Content backend deferred)",
      "Role-gated admin APIs",
    ],
  },
];
