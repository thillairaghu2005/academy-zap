...
export async function recordProgress(
  ...
  if (!lesson) {
    throw new MockApiError("lesson_not_found", "Lesson does not belong to this course.", 404);
  }
  if (!hasCourseAccess(course.id, input.userId)) {
    throw new MockApiError("course_access_required", "Enroll before recording progress.", 403);
  }
