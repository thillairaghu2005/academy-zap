/**
 * Local demo Admin/CMS service (build.md F7).
 *
 ...
 * version — computed by the demo service from the published snapshot.
 */
...
export async function getAdminDashboard(
  ...
  if (actorId === "boom") {
    throw new MockDataError(
      "admin_down",
      "Admin demo data is unavailable (simulated).",
      503,
    );
  }
  seedDemoOrders();
...
async function updateCourseShared(
  ...
  // fields, but the demo service enforces it too — a direct call cannot
...
export async function submitCourseForReview(
  ...
  const course = MOCK_COURSES.find((c) => c.id === courseId);
  if (!course) throw new MockDataError("course_not_found", "Course was not found.", 404);
  if (course.status !== "draft") {
    throw new MockDataError(
      "invalid_transition",
...
export async function publishCourse(
  ...
  // publisher. Enforced by the demo service AND surfaced as a disabled
 ...
 * by the demo service from the published snapshot; the client never diffs or
 * re-derives.
 */
...
export async function setUserRole(
  ...
  if (!user) throw new MockDataError("user_not_found", "User was not found.", 404);
