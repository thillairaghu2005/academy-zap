...
export function SkillTree() {
  ...
  React.useEffect(() => {
    ...
    nodes
      ...
      .attr("r", RADIUS)
      .attr("fill", (d) => {
        // Resolved from design tokens so the tree follows the theme.
        if (!d.parent) return "var(--color-card-tint)";
        const p = d.data.progress ?? 0;
        // Completion green ≥85%, warning amber ≥50%, error red below —
        // lightness + hue differ at every step (colorblind-safe).
        return p >= 0.85 ? "var(--color-success)" : p >= 0.5 ? "var(--color-warning)" : "var(--color-danger)";
      })
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-background/50 backdrop-blur-xl p-6 shadow-sm transition-all hover:border-primary/30">
      <p className="flex items-center gap-2 font-display text-sm font-semibold tracking-wide">
        <GitFork className="size-4 text-primary" /> Skill Tree
      </p>
