...
The two `credential_verify` cases are implemented in `integrity/credentials.py` (slice 08) —
Ed25519-signed W3C Verifiable Credentials, gamification §7.3. The cases were xfailed while
`credentials.py` did not exist, so the gap stayed visible in test output; they turn green now
that the module is real.

This file exists specifically so a future refactor of the resolver/rank/streak/hash-chain
modules can't silently change what these cases mean (build.md §8.2's acceptance-fixture rule) —
the individual unit tests in `tests/unit/` cover broader behavior; this one is the doc's own
literal table, kept in one place for direct traceability.
"""
