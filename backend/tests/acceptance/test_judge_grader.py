...
def test_cases():
    return [
        TestCase(id=uuid.uuid4(), problem_id=uuid.uuid4(), input="hello\n", expected_output="hello\n", position=1),
        TestCase(id=uuid.uuid4(), problem_id=uuid.uuid4(), input="world\n", expected_output="world\n", position=2),
    ]

@pytest.mark.asyncio
async def test_grader_accepted(sandbox, problem, test_cases):
...
@pytest.mark.asyncio
async def test_grader_wrong_answer(sandbox, problem, test_cases):
    grader = Grader(sandbox, problem, test_cases)
...
@pytest.mark.asyncio
async def test_grader_compile_error(sandbox, problem, test_cases):
    grader = Grader(sandbox, problem, test_cases)
    source_code = "def foo( \n" # syntax error
...
@pytest.mark.asyncio
async def test_grader_runtime_error(sandbox, problem, test_cases):
    grader = Grader(sandbox, problem, test_cases)
    source_code = "1 / 0"
